...
if wandb:
    ...
    config = wandb.config
    run = wandb.init(project='zzz', entity='zk', config={'wandb_nb':'wandb_three_in_one_hm'}) 
...
def generate_global_structure(data, K=10):
    ...
    cos_sim = torch.Tensor([cosine_similarity(unit.T) for unit in observations]).to(device)
    ave_sim = torch.mean(cos_sim, dim=0)
    _, indices = torch.sort(ave_sim, dim=0)
    index_K = indices < K
    global_structure = index_K.float() * ave_sim
    return masked_softmax(global_structure)
...
parser.add_argument('--gpu-id', default='7', type=str,help='id(s) for CUDA_VISIBLE_DEVICES')        #1->
...
baseline = False  # always False for 
...
print(args.feature_removal_level)
...
for missing_ratio in missing_ratios:
    ...
    dataset_info = {
        'P12': {'d_static': 9, 'd_inp': 36, 'static_info': 1},
        'P19': {'d_static': 6, 'd_inp': 34, 'static_info': 1},
        'eICU': {'d_static': 399, 'd_inp': 14, 'static_info': 1},
        'PAM': {'d_static': 0, 'd_inp': 17, 'static_info': None}
    }

    d_static = dataset_info[dataset]['d_static']
    d_inp = dataset_info[dataset]['d_inp']
    static_info = dataset_info[dataset]['static_info']

    d_ob = 4                        #4->1，PAM false
    d_model = d_inp * d_ob          #*4
    nhid = 2 * d_model              #*4*2
    nlayers = 2                     #
    nhead = 2                       #
    dropout = 0.2                  #0.1->
    ...
    for k in range(n_splits):
        ...
        Ptrain, Pval, Ptest, ytrain, yval, ytest = get_data_split(base_path, split_path, split_type=split, reverse=reverse,
                                                                  ...
                                                                  predictive_label=args.predictive_label) #len：9590 1199 1199 9590 1199 1199
        if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
            T, F = Ptrain[0]['arr'].shape     #t=215,f=36, d= 9
            D = len(Ptrain[0]['extended_static'])       #dict_keys(['id', 'static', 'extended_static', 'arr', 'time', 'length'])
            ...
            mf, stdf = getStats(Ptrain_tensor)                  ## lenmf,lenstdf=36 mf is the original value and stdf is the normalized value
            ms, ss = getStats_static(Ptrain_static_tensor, dataset=dataset)         #lenms,lenss= 9,   ms9 0，ss9 1

            Ptrain_tensor, Ptrain_static_tensor, Ptrain_time_tensor, ytrain_tensor = tensorize_normalize(Ptrain, ytrain, mf,stdf, ms, ss) 
            #shape [9590, 215, 72])   ([9590, 9])    ([9590, 215, 1])   ([9590])                                                                                                      
        ...
        global_structure = torch.ones(d_inp, d_inp)       ##### d_inp Number of nodes

        # remove part of variables in validation and test set       #zzzzzzzzzzzzzzzzzzzzz
        if missing_ratio > 0:                              #
            num_all_features =int(Pval_tensor.shape[2] / 2)   # Number of nodes
        ...
        Ptrain_tensor = Ptrain_tensor.permute(1, 0, 2)    # The dimension transposition function is replaced by 215,9590，72
        Pval_tensor = Pval_tensor.permute(1, 0, 2)
        # print('Pval_tensor',Pval_tensor.shape)
        Ptest_tensor = Ptest_tensor.permute(1, 0, 2)

        Ptrain_time_tensor = Ptrain_time_tensor.squeeze(2).permute(1, 0)    #  ([215, 9590])
        ...
        for m in range(n_runs):             # 
            ...
            if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':                             
                model = zzz_2(d_inp, d_model, nhead, nhid, nlayers, dropout, max_len,      ##36;36*4;2;36*4*2;2;0.2;215;
                                    d_static, MAX, 0.5, aggreg, n_classes, global_structure,     ##9;100;0.5;mean;2;
                                    sensor_wise_mask=sensor_wise_mask)                           ##
            elif dataset == 'PAM':
                model = zzz_2(d_inp, d_model, nhead, nhid, nlayers, dropout, max_len,
            ...
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1,                    # lr factor从0.1->
                                                                   patience=1, threshold=0.0001, threshold_mode='rel',   #patience从1->
                                                                   cooldown=0, min_lr=1e-8, eps=1e-08, verbose=True)     #  min_lr=  -8  ->

            idx_0 = np.where(ytrain == 0)[0]        #Optimized to one line of code  idx_0=8218
            idx_1 = np.where(ytrain == 1)[0]        #idx_1=1372
            ...
            expanded_idx_1 = np.concatenate([idx_1, idx_1, idx_1], axis=0)         #combine，
            expanded_n1 = len(expanded_idx_1)             #4116

            batch_size = 128
            if strategy == 1:
                n_batches = 10
            elif strategy == 2:
                K0 = n0 // int(batch_size / 2)           #k0=128  The power of two is pure coincidence
                K1 = expanded_n1 // int(batch_size / 2)  #k1=64
            ...
            for epoch in range(num_epochs):
                ...
                if strategy == 2:
                    np.random.shuffle(expanded_idx_1)    # disrupt the order
                    I1 = expanded_idx_1
                ...
                for n in range(n_batches):
                    if strategy == 1:
                        idx = random_sample(idx_0, idx_1, batch_size)        # Random generation
                    elif strategy == 2:
                    ...
                    # P 215,128,72   ptime 215,128    Pstatic 128,9

                    elif dataset == 'PAM':
                        P, Ptime, Pstatic, y = Ptrain_tensor[:, idx, :].cuda(), Ptrain_time_tensor[:, idx].cuda(), \
                                               None, ytrain_tensor[idx].cuda()

                    lengths = torch.sum(Ptime > 0, dim=0)     # 128
                    

                    # print('P, Pstatic, Ptime, lengths',P.shape, Pstatic.shape, Ptime.shape, lengths.shape)
                    outputs, distance, _ = model.forward(P, Pstatic, Ptime, lengths)    ## outputs torch.Size([128, 2])
                    #print('outputs',outputs.shape)

                    optimizer.zero_grad()
                    loss = criterion(outputs, y)+0.25*distance  # ago：loss = criterion(outputs, y)  distance
                    loss.backward()
                    optimizer.step()

                if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
                    train_probs = torch.squeeze(torch.sigmoid(outputs))     
                    train_probs = train_probs.cpu().detach().numpy()      #squeeze   train_probs.shape (128, 2)
                    ...
                    train_probs = torch.squeeze(nn.functional.softmax(outputs, dim=1))     ## outputs  nan solved
                    train_probs = train_probs.cpu().detach().numpy()
                    train_y = y.cpu().detach().numpy()
                    train_auroc = roc_auc_score(one_hot(train_y), train_probs)               ## train_probscd全是0  solved
                ...
                if epoch == 0 or epoch == num_epochs - 1:
                    print(confusion_matrix(train_y, np.argmax(train_probs, axis=1), labels=[0, 1]))  #混淆矩阵

                """Validation"""
                model.eval()
                if epoch == 0 or epoch % 10 == 8:               ## by kkk
                    with torch.no_grad():
                        ...
                        out_val = out_val.detach().cpu().numpy()    #  out_val (1199, 2)
                        ...
                        scheduler.step(aupr_val)           # aupr_val->
